...
<em>The memory footprint of RNA2Drawer continually increases.</em> This seems to be an issue with the underlying Tcl/Tk canvas that the structure editing canvas uses. (Memory is not freed where I had thought it would be.) This is currently being addressed. For now, you can reset the memory footprint of RNA2Drawer if it gets too large by saving your open tabs as RNA2Drawer files and closing/reopening RNA2Drawer and the saved tabs.
