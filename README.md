...
`File` -> `New` allows you to enter a new sequence that will be drawn entirely single-stranded, or enter a new sequence and structure in dot-bracket notation.

`File` -> `Open` allows you to read from a file a plain sequence, a sequence in FASTA format, a sequence and structure in dot-bracket notation, a CT file generated by Mfold, or an RNA2Drawer file containing all the data for a tab.

Make sure what you enter or the file you open agrees with the parsing steps detailed in the entry or open windows.
